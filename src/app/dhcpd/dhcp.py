...
class _DHCPServer(libpydhcpserver.dhcp_network.DHCPNetwork):
        ...
        if not [None for (ignored_mac, timeout) in self._ignored_addresses if mac == ignored_mac]:
            ...
            if sid and not ciaddr: #SELECTING
                ...
                if conf.NAK_RENEWALS:
                    ...
                    if renew:
                        logging.writeLog('DHCPREQUEST:RENEW from %(mac)s on vlan %(vlan)s' % {
                           'mac': mac,
                           'vlan': vlan
                        })
                    else:
                        logging.writeLog('DHCPREQUEST:REBIND from %(mac)s on vlan %(vlan)s' % {
                           'mac': mac,
                           'vlan': vlan
                        })                                                
                    try:
                ...
                logging.writeLog('DHCPREQUEST:UNKNOWN (%(sid)s %(ciaddr)s %(ip)s) from %(mac)s on vlan %(vlan)s' % {
                 'sid': str(sid),
                 'ciaddr': str(ciaddr),
                 'ip': str(ip),
                 'mac': mac,
    ...
    def _sendDHCPPacket(self, packet, address, response_type, mac, client_ip):
        ...
        logging.writeLog('DHCP%(type)s sent to %(mac)s for %(client)s via %(ip)s:%(port)i [%(bytes)i bytes]' % {
             'type': response_type,
             'mac': mac,
             'client': client_ip,
             'bytes': bytes,
