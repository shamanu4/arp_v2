...
class Network(IP):
    ...
    def host_random(self):
        '''
        Random available host in this subnet.
        '''
        import random
        if (self.version() == 4 and self.mask == 32) or (self.version() == 6 and self.mask == 128):
            return self

        return IP(long(random.randint(long(self.network())+1,long(self.broadcast())-1)), version=self.version())

    def host(self,index):
        '''
        Returns available host in this subnet by index.
        '''
        if long(self.network())+index >= long(self.broadcast()):
            raise IndexError
        return IP(long(self.network())+index, version=self.version())
    ...
    def in_network(self, other):
        ...
        '''
        other = Network(other)
        return long(self) <= long(other) < long(self) + self.size() - other.size() + 1
    ...
    def cross(self, net):
        if self.has_key(Network(net).network()) or self.has_key(Network(net).broadcast()):
            return True
        if Network(net).has_key(self.network()) or Network(net).has_key(self.broadcast()):
            return True
        return False
